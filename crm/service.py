...
def validate_parent(user: User) -> bool:
    ...
    if user.external_parent_id:
        try:
            ...
            contact = None
    if not contact:
        res = dyn_get("contacts", params={"$select": "contactid,emailaddress1,firstname,lastname", "$filter": f"emailaddress1 eq '{user.email}'"})
    ...
    for row in links.get("value", []):
        ...
        ParentStudentLink.objects.update_or_create(user=user, student=st, defaults={"active": True, "last_verified_at": timezone.now()})
